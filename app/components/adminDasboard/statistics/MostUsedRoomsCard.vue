    ...
    <template #header>
      <div class="flex items-center justify-between w-full p-4">
        <h3 class="text-lg font-semibold">
          {{ $t('pages.adminDashboard.statistics.mostUsedRooms') }}
        </h3>
        <i class="pi pi-fire text-red-500 text-2xl" />
      </div>
    </template>
    <template #content>
      <div
        v-if="mostUsedRooms.length > 0"
        class="space-y-3"
      >
        <div
          v-for="(room, index) in mostUsedRooms"
          :key="room.id"
...
<script setup>
const { fetchMostUsedRooms, mostUsedRooms } = useStatistics()
